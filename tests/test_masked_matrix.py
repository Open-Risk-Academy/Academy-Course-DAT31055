...
class MMatrix(MaskedArray, npm.matrix,):
    ...
    def __array_finalize__(self, obj):
        npm.matrix.__array_finalize__(self, obj)
...
class TestMaskedMatrix:
    ...
    def test_flat(self):
        ...
        # test simple access
        test = masked_array(npm.matrix([[1, 2, 3]]), mask=[0, 0, 1])
        ...
        control = masked_array(npm.matrix([[3, 2, 1]]), mask=[1, 0, 0])
        ...
        a = masked_array(npm.matrix(np.eye(2)), mask=0)
        b = a.flat
        b01 = b[:2]
    ...
    def test_ravel(self):
        a = masked_array(npm.matrix([1, 2, 3, 4, 5]), mask=[[0, 1, 0, 0, 0]])
        aravel = a.ravel()
    ...
    def test_view(self):
        ...
        a.mask[0] = (1, 0)
        test = a.view((float, 2), npm.matrix)
        assert_equal(test, data)
        assert_(isinstance(test, npm.matrix))
...
class TestSubclassing:
    ...
    def test_maskedarray_subclassing(self):
        ...
        (x, mx) = self.data
        assert_(isinstance(mx._data, npm.matrix))
    ...
    def test_masked_binary_operations(self):
        ...
        assert_(isinstance(add(mx, mx)._data, npm.matrix))
