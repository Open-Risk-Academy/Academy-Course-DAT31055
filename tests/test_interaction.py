...
def test_fancy_indexing():
    ...
    m = npm.matrix([[1, 2], [3, 4]])

    assert_(isinstance(m[[0, 1, 0], :], npm.matrix))
...
def test_array_astype():
    ...
    a = npm.matrix([[0, 1, 2], [3, 4, 5]], dtype='f4')
    ...
    assert_equal(a, b)
    assert_equal(type(b), npm.matrix)

    # subok=False never returns a matrix
    b = a.astype('f4', subok=False, copy=False)
    assert_equal(a, b)
    assert_(not (a is b))
    assert_(type(b) is not npm.matrix)


def test_stack():
    # 2018-04-29: copied here from core.tests.test_shape_base
    # check npm.matrix cannot be stacked
    m = npm.matrix([[1, 2], [3, 4]])
...
def test_ediff1d_matrix():
    ...
    assert (isinstance(np.ediff1d(npm.matrix(1)), npm.matrix))
    assert (isinstance(np.ediff1d(npm.matrix(1), to_begin=1), npm.matrix))
...
def test_kron_matrix():
    ...
    assert_equal(type(np.kron(m, m)), npm.matrix)
    assert_equal(type(np.kron(a, m)), npm.matrix)
    assert_equal(type(np.kron(m, a)), npm.matrix)
...
class TestConcatenatorMatrix:
    ...
    def test_matrix(self):
        ...
        assert_equal(type(ab_r), npm.matrix)
        assert_equal(type(ab_c), npm.matrix)
